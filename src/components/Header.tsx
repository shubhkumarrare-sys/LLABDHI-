...
export const Header: React.FC<HeaderProps> = ({
  ...
  // Pending and upcoming obligations from Google Sheets
  const pendingCompliance = compliance.filter((c) => c.status !== 'Filed');
  const pendingEmis = emis.filter((e) => e.status !== 'Paid');
  const totalAlertsCount = pendingCompliance.length + pendingEmis.length;

  // Tab Title Mapping for only active supported tabs
  const tabTitles: Record<string, { title: string; subtitle: string }> = {
    ...
    emis: {
      ...
      subtitle: 'Monthly Bank Installments, Loan Facilities & Repayment Records',
    },
    compliance: {
      title: 'Statutory LLP Compliance',
      subtitle: 'GST, MCA V3 & Income Tax Statutory Schedule',
    },
    scripting: {
    ...
    settings: {
      title: 'Executive Settings & Sync',
      subtitle: 'Google Sheet Connection & Data Backup',
    },
  };
      ...
      <div className="flex items-center space-x-3 sm:space-x-4 min-w-0">
        ...
        <div className="truncate">
          <div className="flex items-center space-x-2 text-[11px] text-[#7D8499] font-medium hidden sm:flex">
            ...
            <span>/</span>
            <span className={`inline-flex items-center space-x-1 ${isLiveConnected ? 'text-emerald-600' : 'text-rose-600'} font-semibold`}>
              <span className={`w-1.5 h-1.5 rounded-full ${isLiveConnected ? 'bg-emerald-500' : 'bg-rose-500'}`} />
              <span>{isLiveConnected ? 'Live Sheet' : 'Reconnecting...'}</span>
            </span>
          </div>
      ...
      <div className="flex items-center space-x-2 sm:space-x-3 shrink-0">
        ...
        {onSearchChange && (
          <div className="relative hidden md:block">
            ...
            <input
              ...
              placeholder="Search loans, compliance, returns..."
        ...
        {onRefreshSheet && (
          <button
            ...
            title={lastUpdatedTime ? `Last updated: ${lastUpdatedTime}. Click to force sync.` : 'Refresh Live Google Sheet Data'}
          ...
          >
            ...
            <span className="hidden xl:inline text-xs">
              {isRefreshingSheet ? 'Syncing...' : lastUpdatedTime ? `Sync (${lastUpdatedTime})` : 'Sync Sheet'}
            </span>
          </button>
        )}
              ...
              <div className="flex items-center justify-between border-b border-[#E8EBF2] pb-3">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider">
                    Executive Obligations ({totalAlertsCount})
                  </span>
                </div>
              ...
              <div className="max-h-64 overflow-y-auto divide-y divide-[#E8EBF2] my-2">
                {totalAlertsCount === 0 ? (
                  <div className="py-6 text-center text-xs text-[#7D8499]">
                    ...
                    All statutory compliance returns and EMIs are cleared!
                  </div>
                ) : (
                  <>
                    {/* Compliance alerts */}
                    {pendingCompliance.slice(0, 4).map((c) => (
                      <div
                        key={c.id}
                      ...
                      >
                        <div className="min-w-0 pr-2">
                          <div className="flex items-center space-x-1.5">
                            <FileCheck2 className="w-3.5 h-3.5 text-[#3045F5] shrink-0" />
                            <span className="text-xs font-bold text-[#171B3A] truncate">
                              {c.title}
                            </span>
                          </div>
                          <div className="flex items-center space-x-2 text-[10px] text-[#7D8499] mt-0.5">
                            ...
                            <span>•</span>
                            <span className="text-rose-600 font-semibold">Due {c.dueDate}</span>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          className="text-[10px] text-[#3045F5] font-semibold hover:underline inline-flex items-center space-x-0.5 shrink-0"
                        >
                          <span>View</span>
                    ...
                    {pendingEmis.slice(0, 3).map((e) => (
                      ...
                      >
                        <div className="min-w-0 pr-2">
                          ...
                          <div className="flex items-center space-x-2 text-[10px] text-[#7D8499] mt-0.5">
                            <span>{formatINR(e.monthlyEmi)}</span>
                            <span>•</span>
                            <span className="text-amber-700 font-semibold">Due {e.nextDueDate}</span>
                          </div>
                        </div>
